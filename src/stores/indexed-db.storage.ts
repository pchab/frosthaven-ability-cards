...
function getClass<X extends Card>() {
	const selectedClassName = classURIToName(
		document.location.pathname.split("/")[1] as FrosthavenClass<X>["name"],
	);
	return frosthavenClasses.find(
		({ name }) => name === selectedClassName,
	) as FrosthavenClass<X>;
}

function departializeCardForClass<X extends HiveCard>(
...
export const indexedDBStorage: StateStorage = {
	getItem: async (): Promise<string | null> => {
		...
		const { cards, availableCards, states, ...state } = result;
