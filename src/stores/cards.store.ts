...
export type SelectedActions = [Action | undefined, Action | undefined];
...
export type PersistedCard = Pick<
	Card,
	"name" | "status" | "tokenPosition" | "enhancements"
> &
...
export const useFrosthavenStore = create<
	AbilityCardsState & AbilityCardsActions
>()(
	persist(
		(set) => ({
			...initialState,
			...
			validateCardSelection: () =>
				set(({ cards }) => ({ states: [cards], currentStateIndex: 0 })),
			enhanceCard: (card: Card) =>
				set(({ cards, availableCards }) => ({
					cards: cards.with(
						cards.findIndex(({ name }) => name === card.name),
						card,
					),
					availableCards: availableCards.with(
						availableCards.findIndex(({ name }) => name === card.name),
						card,
					),
				})),
			...
			updateStates: <X extends Card>(states: X[][]) =>
				set({ states, currentStateIndex: states.length - 1 }),
			...
			reset: async () => {
				...
				const { state } = JSON.parse(stateString) as {
					state: AbilityCardsState;
				};
				set(state);
			},
		}),
