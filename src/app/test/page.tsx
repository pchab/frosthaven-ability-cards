...
export default function TestCard(props: {
	...
	const { className = bannerSpear.name, cardName = bannerSpearCards[0].name } =
		searchParams;

	const selectedClass = frosthavenClasses.find(
		({ name }) => name === className,
	);
	const selectedCard =
		selectedClass?.cards.find(({ name }) => name === cardName) ??
		selectedClass?.cards[0];
	...
	const addEnhance = (index: number, enhanceName: Enhancement) => {
		...
		const newEnhancements = [
			...(newCard.enhancements ??
				Array.from({ length: newCard.availableEnhancements?.length ?? 0 })),
		];
	...
	return (
		<div className="flex flex-col p-32 items-center w-full gap-4">
			<select
			...
			>
				{frosthavenClasses.map((fhClass) => {
					return (
						<option key={fhClass.name} value={fhClass.name}>
							{fhClass.name}
						</option>
					);
				})}
