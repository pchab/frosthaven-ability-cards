...
const geminateCheckHandsize: CheckHandSize<GeminateCard> = (
	cards: GeminateCard[],
) =>
	cards.filter((card) => card.form === "melee").length <=
		geminate.handSize / 2 &&
	cards.filter((card) => card.form === "ranged").length <=
		geminate.handSize / 2;

export function useSelectCards<X extends Card>() {
	const selectedClass = use(ClassContext);
	const { cards, selectCards, enhanceCard } = useFrosthavenStore(
		useShallow((state) => ({
	...
	const selectCard = (card: X) => {
		...
		if (
			!isGeminateCards(selectedClass, newSelectedCards) &&
			newSelectedCards.length <= selectedClass.handSize
		) {
		...
		if (
			isGeminateCards(selectedClass, newSelectedCards) &&
			geminateCheckHandsize(newSelectedCards)
		) {
			return selectCards(newSelectedCards);
		}
	};

	const removeCard = (card: X) =>
		selectCards(cards.filter(({ name }) => name !== card.name));
