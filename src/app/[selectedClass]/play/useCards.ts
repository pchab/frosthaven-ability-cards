...
import type { Card, CardStatus } from "@/domain/cards.type";
import { useFrosthavenStore } from "@/stores/cards.store";

export type Action = "top" | "bottom" | "default";

function newStatusAfterAction(
...
export function useCards<X extends Card>() {
	const { states, updateStates, currentStateIndex, setStateIndex } =
		useFrosthavenStore(
			useShallow((store) => ({
				states: store.states,
	...
	const changeCardStatus =
		(newStatus: CardStatus, condition?: () => boolean) => (card: X) => {
	...
	const playCards = (cardsPlayed: { action: Action; card: X }[]) => {
		const newState = [
			...currentCards.filter(
				(card) => !cardsPlayed.map(({ card }) => card.name).includes(card.name),
			),
			...cardsPlayed.map(({ action, card }) => ({
				...card,
				status: newStatusAfterAction(card.actions, action),
			})),
		];
