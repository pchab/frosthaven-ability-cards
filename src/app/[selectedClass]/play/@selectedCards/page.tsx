...
import { type Action, useCards } from "@/app/[selectedClass]/play/useCards";
import type { Card } from "@/domain/cards.type";
import { type SelectedActions, useFrosthavenStore } from "@/stores/cards.store";

const bottomActionMask = (
	<div
		key={"mask-action-top"}
...
export default function PlayedCards<X extends Card>() {
	...
	const { selectedActions, setSelectedActions } = useFrosthavenStore(
		useShallow((store) => ({
			selectedActions: store.selectedActions,
			setSelectedActions: store.setSelectedActions,
		})),
	);
	const { connectionStatus, setGhsInactive, setGhsInitiative } = useSecretary();

	const selectedCards = currentCards.filter(
			...
			? [
					{
						name: "End Turn",
						node: (
							<button type="button" autoFocus onClick={endTurn}>
								End Turn
							</button>
						),
					},
	...
	return (
		<BoardArea title="Selected cards" actions={actions}>
					...
					{selectedCards.map((card, index) => (
						...
						>
							{selectedActions[index] &&
								getSelectedActionMasks(selectedActions[index])}
						</CardComponent>
					))}
				</AnimatePresence>
			</div>
