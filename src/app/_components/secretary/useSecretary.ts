...
import type {
	...
	Figure,
	GameState,
} from "@/domain/secretary/game.state";
...
function updateGameStateForFigure<F extends Figure>(
	...
	const { characters, monsters, revision, playSeconds, ...rest } = oldGameState;

	const currentCharacterIndex = characters.findIndex(
		({ name }) => name === figureName,
	);
	const currentMonsterIndex = monsters.findIndex(
		({ name }) => name === figureName,
	);
...
export default function useSecretary<X extends Card>() {
	const { connectionStatus, gameState, sendGameStateToGhs } =
	...
	const [lastUpdateInSeconds, setLastUpdateInSeconds] = useState<number>(
		getNowInSeconds(),
	);
	...
	const setGhsInitiative = ({ initiative }: X) => {
		...
		const newGameState = updateGameStateForFigure(
			gameState,
		...
		if (sendGameStateToGhs) {
			sendGameStateToGhs(newGameState, [
				"setInitiative",
				currentCharacter.title || currentClass.name,
				`'${newInitiative}'`,
			]);
		}
	};
	...
	const setGhsInactive = () => {
		if (connectionStatus !== WebSocket.OPEN || !currentClass || !gameState)
			return;
		...
		if (!currentCharacter || currentCharacter.off || !currentCharacter.active)
			return;
		newGameState = updateGameStateForFigure(
			gameState,
			currentCharacter.name,
	...
	useEffect(() => {
		...
		const currentCharacter = gameState.characters.find(
			({ name }) => name === mapCharacterNameToSecretary(currentClass.name),
		);
		const currentPlayingFigure = [
