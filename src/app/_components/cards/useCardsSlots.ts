...
export function useCardsSlots<X extends Card>() {
	const { states, updateStates, currentStateIndex } = useFrosthavenStore(
		useShallow((store) => ({
			states: store.states,
	...
	const moveTokenForward = (card: X) => {
		...
		if (newTokenPosition >= card.slots.length) {
			...
			const newStatus =
				card.actions[action] === "activeDiscard" ? "discarded" : "lost";
			const newState = currentCards.with(cardIndex, {
				...card,
				status: newStatus,
				tokenPosition: 0,
			});
	...
	const moveTokenBackward = (card: X) => {
		...
		const cardIndex = currentCards.indexOf(card);
		const newTokenPosition = Math.max(
			card.tokenPosition ? card.tokenPosition - 1 : 0,
			0,
		);
		const newState = currentCards.with(cardIndex, {
			...card,
